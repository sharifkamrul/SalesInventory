...
public class WearHouse extends Product {
	
	WMySQLAccess dbAccess = new WMySQLAccess();
	...
	public void prodPurchase(String prodName, String prodCode, String barCode, 
		...
		try {
			...
			sql = dbAccess.insertToWearHouseProducts(prodName, prodCode, barCode, brandName, purchaseRate, sellingRate, quantity);
	...
	public Object[][] showCurrentStock(String ac) {
		...
		try {
			...
			if(ac.equals(Product.AC_CURRENTSTOCK)) {
				...
				sql = dbAccess.selectAll("wear_house_products", null, column, null);
			}
			else if(ac.equals(Product.AC_STOCKOUTINFO)) {
				sql = dbAccess.selectAll("wear_house_products", "wearhouse_stockout_info", column, "prodCode");
			}
			else {}
			...
			if(ac.equals(Product.AC_CURRENTSTOCK)) {
				...
				sql = dbAccess.selectAll("wear_house_products", null, null, null);
			}
			else if(ac.equals(Product.AC_STOCKOUTINFO)) {
				column[0] = "wear_house_products.prodName";
				column[1] = "wear_house_products.prodCode";
				column[2] = "wear_house_products.brandName";
				column[3] = "wearhouse_stockout_info.purchaseRate";
				column[4] = "wearhouse_stockout_info.sellingRate";
				column[5] = "wearhouse_stockout_info.quantity";
				column[6] = "wearhouse_stockout_info.Date";
	...
	public void stockOut(String prodCode, int stockOutQuantity, String sRoomName) {
		try {
			...
			sql = dbAccess.updateWearHouseProducts("quantity", new_quantity);
			sql+= " where prodCode='"+prodCode+"'";
			//System.out.println(sql);
			stmt.executeUpdate(sql);
			
			//	insert relevant info in wear_house_stock_info table to keep stock out info*/
			sql = dbAccess.insertToWearHouseStockOUTInfo(prodCode, purchaseRate, sellingRate, stockOutQuantity, sRoomName);
			//System.out.println(sql);
			stmt.executeUpdate(sql);
